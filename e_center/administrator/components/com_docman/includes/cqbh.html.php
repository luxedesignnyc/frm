...
class HTML_DMCQBH
{
    function show(&$rows, $myid, &$pageNav, &$lists, $type)
    {
		...
		<tr>
			...
			<th class="title"><font color="white"><?php echo _DML_CQBHNAME;?></font?</th>
        ...
        foreach ($rows as $row) {
            ...
            $task = $row->public ? 'unpublish' : 'publish';
            $alt = $row->public ? 'Published' : 'Unpublished';
            if (!$row->access) {
		...
		<input type="hidden" name="section" value="cqbh" />
