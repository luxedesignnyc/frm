...
class DOCMAN_FileUpload
{
	...
	*/
	...
	/*BEGIN: TUYEN wrote functions here*/
	
	function uploadHTTP_New(&$file, $path, $lates_version, $document_id, $validate = _DM_VALIDATE_ALL )
    {
		$name 	   = DOCMAN_Utils::stripslashes($file['name']);
		...
		if( $errorcode == 0 ){
    	     return $this->_upload_New($name, $temp_name, $path, $lates_version, $document_id);
        }
	...
	function _upload_New ($name, $temp_name, $path, $lates_version, $document_id) {
	  				
		if (is_uploaded_file($temp_name)) {
			$file = new DOCMAN_File($name, $path);
			$strExt = "." . $file->getExtension();
			$name = $document_id."_".str_replace($strExt, "", $name). "_v" . $lates_version .$strExt;
	   		if (move_uploaded_file($temp_name, $path."/".$name)) {
    			$file->DOCMAN_File($name, $path);
               	return $file;
	   		} else {
        		$this->_err = _DML_DIRPROBLEM." ";
	   		}
	...
	/*function insertFilesTable ($doc_id) {
		global $database;
		try {
			//Select to check and create new Version.
			$query = ' SELECT license_name' .
					 ' FROM #__docman_files'.
					 ' WHERE doc_id = '. (int) $doc_id.
					 ' ORDER BY created_date';
			$database->setQuery($query);
			$floatLicense = $database->loadObject(); 

			
			//Ins
			$user		= & JFactory::getUser(); 		
			$query = " INSERT INTO `#__docman_files` (`doc_id`, `license_name`, `file_url`, `created_by_id`, `created_date`, 
													  `approved_by_id`, `approved_date`, `effective_date`, `expiration_date`, 
													  `track`, `status`, `file_type`, `reason`)".
					 " VALUES 						 ('" .$doc_id. "','" . "1.2.3.4" . "','" . $name . "','" . $user->get('id') . "','" . getDate() . "','" .
					 "')";
			$database->setQuery($query);
			...
			$this->_err = $database->getErrorMsg();
		}	
	}*/
	/*END: TUYEN wrote here*/
