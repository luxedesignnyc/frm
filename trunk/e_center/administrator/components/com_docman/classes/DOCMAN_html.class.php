...
class dmHTML extends mosHTML
{
	...
	// List co quan ban hanh
	function CQBHcategoryList($id, $action, $options = array())
    {
        global $_DOCMAN;
        require_once($_DOCMAN->getPath('classes', 'utils'));
        ...
        $parent = mosHTML::selectList($options, 'cqbhid', 'id="cqbhid" class="inputbox" size="1" onchange="' . $action . '"', 'value', 'text', $id);
        return $parent;
    }
	
	// List Loai van ban
	function LVBcategoryList($id, $action, $options = array())
    {
        global $_DOCMAN;
        require_once($_DOCMAN->getPath('classes', 'utils'));
        $list = DOCMAN_utils::LVBcategoryArray();
        // assemble menu items to the array
        foreach ($list as $item) {
            $options[] = mosHTML::makeOption_New($item->id, $item->treename, "id = ".$item->catid);
        }
        $parent = mosHTML::selectList($options, 'lvbid', 'id="lvbid" class="inputbox" size="1" onchange="' . $action . '"', 'value', 'text', $id);
	...
	function CQBHcategoryParentList($id, $action, $options = array())
    {
        global $_DOCMAN, $database;
        require_once($_DOCMAN->getPath('classes', 'utils'));
        $list = DOCMAN_utils::CQBHcategoryArray();
        ...
        // $cat = new mosDMCategory($database);
        // $cat->load($id);
        // Class docman.class.php
        $cat = & mosDMCQBH::getInstance( $id );
    ...
    // Loai van ban
    function LVBcategoryParentList($id, $action, $options = array())
    {
        global $_DOCMAN, $database;
        require_once($_DOCMAN->getPath('classes', 'utils'));
        $list = DOCMAN_utils::LVBcategoryArray();
