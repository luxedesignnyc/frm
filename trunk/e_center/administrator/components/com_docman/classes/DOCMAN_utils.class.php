...
class DOCMAN_Utils
{
 	...
 	function CQBHcategoryArray()
    {
        ...
        $query = "SELECT c.*, c.parent_id AS parent "
         ...
         . "\n WHERE public  AND access <= ".$_DMUSER->gid
         . "\n ORDER BY ordering" ;
    ...
    // Ham tra ve ten co quan ban han
    function getCQBHName($docid)
    {
        global $database, $_DMUSER;
 		$result = "Hello";
        // get a list of the menu items
        $query = "SELECT c.name "
         . "\n FROM #__docman d"
         . "\n LEFT JOIN #__docman_cqbh AS c ON c.id = d.cqbhid"
         . "\n WHERE d.id = ".$docid
         . "\n ORDER BY ordering" ;
	...
	//TUYEN: Get Lates Version
	function getLatesVersion ($doc_id) {
		global $database;
		...
		return (($floatLatestLicense == 0) ? 1.0 : $floatLatestLicense + 0.1);
	}
	
	//TUYEN: Saving database in jos_docman_files after Uploadinig file
	function saveInFileTable ($doc_id, $latest_version, $file_url, $size, $file_type, $reason, $status) {
		global $database;
		$user		= & JFactory::getUser(); 
		$__query = " INSERT INTO `#__docman_files` (`doc_id`, `license_name`, `file_url`, `created_by_id`, `created_date`, `size`, `file_type`, `reason`, `status`)".
				   " VALUES 					   ('" . $doc_id . "','" . $latest_version . "','" . $database->getEscaped($file_url) . "','" . $user_id . "', NOW(), '". $size . "','" . $file_type . "','" . $reason . "','" . $status . "')";
	...
	//This function return files' information of documents
	function getFilesByDocID($docid) {
        global $database;
        // get a list of the menu items
        $query = " SELECT * " .
         		 " FROM #__docman_files".
 	...
 	function LVBcategoryArray()
    {
        ...
        $query = "SELECT c.*, c.parent_id AS parent "
         . "\n FROM #__docman_lvb c"
         . "\n WHERE published  AND access <= ".$_DMUSER->gid
         . "\n ORDER BY ordering" ;
    ...
    // Ham tra ve ten loai van ban
    function getLVBame($docid)
    {
        global $database, $_DMUSER;
 		$result = "Hello";
        // get a list of the menu items
        $query = "SELECT c.name "
         . "\n FROM #__docman d"
         . "\n LEFT JOIN #__docman_lvb AS c ON c.id = d.lvbid"
         . "\n WHERE d.id = ".$docid
         . "\n ORDER BY ordering" ;
    ...
    function taskLink($task, $gid = '', $params = null, $sef = true, $indexfile = 'index.php')
    {
		...
		if ($task == "doc_download_by_Tuyen") return "javascript: downloadButtonFront(".$gid.")"; //TUYEN added this command
...
class DOCMAN_Cats
{
    ...
    */
    ...
    /////////////// Danh cho CQBH
 	function getChildsByUserAccess_CQBH($parent_id = 0, $ordering = "ordering ASC", $userID = null)
    {
        global $database, $_DOCMAN;
        ...
        $query = "SELECT * FROM #__docman_cqbh "
         . "\n WHERE  public = 1 "
         . "\n   AND parent_id=". (int) $parent_id ." AND ";
    ...
    {
        $cat = new StdClass();
        // TUYENTN $cat->name  = $cats[$id]->name;
       //TUYENTN $cat->title = $cats[$id]->title;
        $sef = defined('_DM_J15') ? false : true;
        $cat->link = DOCMAN_Utils::taskLink('cat_view', $id, null, $sef);
        $ancestors[] = &$cat;
		$id =0;
        // TUYENTN $id = $cats[$id]->parent;
        if ($id != 0) {
     ...
     * Returns an array of category objects with their id as key
     */
    function & getCQBHCategoryList(){
    	static $list;

        if( !isset($list) ) {
        	global $database;
            $database->setQuery( "SELECT * FROM #__docman_cqbh" );
            $list = $database->loadObjectList( 'id' );
        }
        return $list;
    }
     /**
     * Returns loai van ban
     */
    function & getLVBCategoryList(){
    	static $list;

        if( !isset($list) ) {
        	global $database;
            $database->setQuery( "SELECT * FROM #__docman_lvb" );
            $list = $database->loadObjectList( 'id' );
        }
        return $list;
    }
...
class DOCMAN_Docs
{
    ...
    */
    function getDocsByUserAccess($catid = 0, $ordering = '', $direction = '', $limit = '', $limitstart = 0)
    {
        ...
        $query = "SELECT d.*, c.title AS cat_title FROM #__docman AS d"
        	. "\n LEFT JOIN #__docman_cqbh AS c ON d.catid = c.id ";
  ...
  //////////////
  function getDocsByUserAccess_new($gid = 0, $op='', $ordering = '', $direction = '', $limit = '', $limitstart = 0)
    {
        global $database, $_DOCMAN;
        ...
        if ($op == 'cqbh' ){
			$query = "SELECT d.*, c.name AS cat_title FROM #__docman AS d"
				   . "\n LEFT JOIN #__docman_cqbh AS c ON d.cqbhid = c.id ";
        }else{
			$query = "SELECT d.*, c.name AS cat_title FROM #__docman AS d"
				   . "\n LEFT JOIN #__docman_lvb AS c ON d.lvbid = c.id ";
        }
	...
	//BEGIN: TUYEN - this function get Document after clicking menu
	/**
	 * $gid: Id of section or category
	 * $op: value is  "sec" or "cat"
	**/
	function getDocsByUserAccess_ClickMenu ($gid = 0, $op='', $ordering = '', $direction = '', $limit = '', $limitstart = 0)
    {
        global $database, $_DOCMAN;
		...
		// preform query
        $query = " SELECT d.* FROM #__docman AS d".
				 " INNER JOIN #__docman_sec_cat AS dsc ON dsc.doc_id = d.id ";
		
        if (!$user->userid) {//DON'T REGISTER
         	if(!$_DOCMAN->getCfg('registered')) {//TUYEN: $registered = 0 -> Must Login to view, download any Document
         		return array();
         	}

            $query .= "WHERE d.dmowner=" . _DM_PERMIT_EVERYONE
                   . "\n AND d.published=1 AND d.approved=1";
		   if ($op == 'sec') {//Tobe SECTION
		   		$query .= $gid ? "\n AND dsc.section_id IN ($gid)" : "";
		   } elseif ($op == 'cat'){//CATEGORY
           		$query .= $gid ? "\n AND dsc.category_id IN ($gid) " : "";
           }
	...
	//END: TUYEN - this function get Document after clicking menu
	
	
    /////
   function getDocsByUserAccessAdmin( $ordering = '', $direction = '', $limit = '', $limitstart = 0)
    {
        global $database, $_DOCMAN;
        ...
        $query = "SELECT d.*, c.name AS cat_title FROM #__docman AS d"
        	. "\n LEFT JOIN #__docman_cqbh AS c ON d.cqbhid = c.id "
        	. "\n WHERE d.published=0 OR d.approved=0";
   ...
   //// 	
    function getDocsByUserAccess_lastnew( $ordering = '', $direction = '', $limit = '', $limitstart = 0)
    {
        global $database, $_DOCMAN;
   ...
   ////////////// 
  function getDocsByUserAccess_All($gid = 0, $op='', $ordering = '', $direction = '', $limit = '', $limitstart = 0)
    {
        global $database, $_DOCMAN;
        ...
        $query = "SELECT d.*, c.name AS cat_title FROM #__docman AS d"
        	. "\n LEFT JOIN #__docman_cqbh AS c ON d.cqbhid = c.id "
        	. "\n LEFT JOIN #__docman_lvb AS l ON d.lvbid = l.id ";
     ...
     //////////////
     /*
      Tim van ban theo thang, nam 
     */
    function getDocsByUserAccessForMonthYear($dmmonth = 0, $dmyear=0, $ordering = '', $direction = '', $limit = '', $limitstart = 0)
    {
        global $database, $_DOCMAN;
		...
		$query .= " AND d.dmmonth ='$dmmonth'";
		$query .= " AND d.dmyear ='$dmyear'";
    ...
    ///////////////////
    /*
    Dem so luong van ban trong mot nam	
    */
    function getDocsByYear($ordering = '', $direction = '', $limit = '', $limitstart = 0)
    {
        global $database, $_DOCMAN;
        ...
        $query = "SELECT count(id) so_vb,dmyear FROM #__docman "
				 . "\n	group by dmyear "
				 . "\n	order by dmyear DESC";
    ...
    //////////
	/*
	|| Dem so luong ban ban trong thang cua mot nam
	*/
     function getDocsByMonth( $year, $ordering = '', $direction = '', $limit = '', $limitstart = 0)
    {
        global $database, $_DOCMAN;
        ...
        // preform query
        	 ...
        	 $query = "SELECT count( id ) so_vb, dmyear, dmmonth "
				 ."\n	FROM #__docman "
				 ."\n	WHERE dmyear ='$year' "
				 ."\n	GROUP BY dmyear,dmmonth "
				 ."\n	ORDER BY dmyear DESC,dmmonth ASC ";
    ...
    */
    function search(&$searchArray, $ordering = '', $cats = '', $columns = '', $options = array(),$cqbhid ='',$lvbid='')
    {
