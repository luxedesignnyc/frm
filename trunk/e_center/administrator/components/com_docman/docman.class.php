...
class mosDMDocument extends mosDBTable {
    ...
    var $attribs            = null;
    var $ngay_ky            = null;
    var $ngay_hieu_luc      = null;
    var $ngay_het_hieu_luc  = null;
    var $nguoi_ky           = null;
    var $dmmonth           = null;
    var $dmyear           = null;
	...
	*/
	...
	//	$this->dmdate_published = date( "Y-m-d H:i:s" );
		...
		$this->ngay_ky = date( "Y-m-d" );
	//	$this->ngay_duyet = date( "Y-m-d" );
	//	$this->ngay_hieu_luc = date( "Y-m-d" );
	//	$this->nguoi_duyet = "";
		//$this->ngay_het_hieu_luc = date( "Y-m-d" );
	...
	*/
	...
	function check()
	{
		...
		*/
		...
		$this->dmowner = -1;//TUYEN: rule - Everybody
  	    $this->dmmantainedby = -6;//TUYEN: rule - Editor
		...
		*/
        ...
        // making sure...
        $this->id                 = ((int) $this->id == -1?0:(int) $this->id);
        $this->catid              = (int) $this->catid;
        $this->cqbhid              = (int) $this->cqbhid;
        $this->lvbid              = (int) $this->lvbid;
		...
		$this->ngay_ky            = strip_tags( $this->ngay_ky );
    	$this->ngay_hieu_luc      = strip_tags( $this->ngay_hieu_luc);
        $this->ngay_het_hieu_luc  = strip_tags( $this->ngay_het_hieu_luc);
    	$this->nguoi_ky			  = strip_tags( $this->nguoi_ky);
		...
		if( ! $this->ngay_ky )	$this->ngay_ky = $date;
		else $this->ngay_ky = date( 'Y-m-d', strtotime($this->ngay_ky)); 
		
		if ($this->ngay_hieu_luc) $this->ngay_hieu_luc = date( 'Y-m-d', strtotime($this->ngay_hieu_luc));		
		else $this->ngay_hieu_luc = null;
		
		if ($this->ngay_het_hieu_luc) $this->ngay_het_hieu_luc = date('Y-m-d', strtotime($this->ngay_het_hieu_luc));
		else $this->ngay_het_hieu_luc = null;
		
		$this->dmlastupdateon 	= $date;

		$dmyear =			getdate (strtotime($this->ngay_ky));
	
	   	$this->dmmonth			  = $dmyear[mon];
    	$this->dmyear			  = $dmyear[year];
	...
	*/
	...
	function save() {
		...
		//BEGIN: TUYEN - Insert in jos_docman_sec_cat TABLE
		//DELETE
		$query =  " DELETE FROM `#__docman_sec_cat`".
				  " WHERE doc_id ='". $this->id ."'";
		$database->setQuery($query);
		$database->query();				  
		//UPDATE 		
		$user		= & JFactory::getUser();
		$query =  " INSERT INTO `#__docman_sec_cat` (`section_id`, `category_id`, `doc_id`, `track`, `created_date`)".
                  " VALUES  ('" . $this->catid . "','" . $this->lvbid . "','" . $this->id . "','" . $user->get('id') . "', NOW())";
			
		$database->setQuery($query);
		$database->query();
		//END: TUYEN - Insert in jos_docman_sec_cat TABLE
...
/**
* Cqbh database table class
* @package DOCman_1.4
*/
class mosDMCQBH extends mosDBTable {
	...
	* @param database A database connector object
	*/
	function mosDMCQBH( &$db ) {
		$this->mosDBTable( '#__docman_cqbh ', 'id', $db );
	}
    ...
    function & getInstance( $id = 0 ) {
        ...
        if( !$id) {
        	$new = new mosDMCQBH( $database );
            return $new;
        }

        if( !isset( $instances[$id] )) {

            $instances[$id] = new mosDMCQBH( $database );
            //$instances[$id]->load( $id );

            // instead of loading, we'll use the the following method to improve performance
            $list = & DOCMAN_Cats::getCQBHCategoryList(); // get a list of categories, $list[$id] is our current category
            $instances[$id]->bind( (array) $list[$id] ); // assign each property of the category to the category object
        }
...
* @package DOCman_1.4
*/
class mosLVB extends mosDBTable {
	...
	*/
	function mosLVB( &$db ) {
		$this->mosDBTable( '#__docman_lvb ', 'id', $db );
	}
    ...
    function & getInstance( $id = 0 ) {
        ...
        if( !$id) {
        	$new = new mosLVB( $database );
            return $new;
        }

        if( !isset( $instances[$id] )) {

            $instances[$id] = new mosLVB( $database );
            //$instances[$id]->load( $id );

            // instead of loading, we'll use the the following method to improve performance
            $list = & DOCMAN_Cats::getLVBCategoryList(); // get a list of categories, $list[$id] is our current category
            $instances[$id]->bind( (array) $list[$id] ); // assign each property of the category to the category object
        }
