...
var count = -1;
var catObjArray = new Array();
var upDown = true;//TRUE: add at top, FALSE add at Botton
//TUYEN: 

function insertArrray (childId) {

	var catObj = document.getElementById(childId);
					
	var i = 0;
	for (i=0; i<catObj.options.length; i++) {		
		  catObjArray[i] = catObj.options[i];
	}
}
...
function loopSelected(parentId, childId)	{
	...
	var length = catObjArray.length;	  

	for (i=0; i<length; i++) {
		catObj.options[i] = catObjArray[i];
	}
